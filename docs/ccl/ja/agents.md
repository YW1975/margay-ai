...
- 組み込みエージェントには general-purpose、code-reviewer、test-runner、Explore、Plan、verification、statusline setup、有効な場合の CCL guide ロールがあります。
- カスタムおよびプラグイン由来のエージェントは定義ディレクトリから読み込まれ、ツール、モデル、MCP 要件、hook、権限、メモリ、バックグラウンド動作を制限できます。
- エージェントは前景、背景、または teammate 型 pair-agent path で実行でき、対応している場合はメッセージで継続できます。
...
- `/buddy` command は Agent tool の teammate path を呼び出す convenience prompt です。`team_name` と teammate `name` を渡し、helper が team channel で lead session と協調できるようにします。
...
| Code reviewer | commit、PR、引き渡し前に変更を集中的にレビューする | bug risk、regression、security、missing test の確認に使います。 |
| Test runner | test command または harness run を実行し要約する | 出力が長い場合、または concise pass/fail report が必要な場合に使います。 |
